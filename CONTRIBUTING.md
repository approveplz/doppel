...
needed. Tests use the same approval requests as `/hooks` within their temporary
homes. They do not bypass hook trust. Setup is tested before any model turn,
including rerunning after approval and restoring fallback on uninstall.
...
Run `python3 plugins/doppel/scripts/doppel.py setup --source /path/to/doppel`
from your checkout. Follow the printed hook approval instructions, then rerun
the command. Start a new Codex session after activation.
